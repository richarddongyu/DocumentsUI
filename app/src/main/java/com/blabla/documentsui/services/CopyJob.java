...
import com.blabla.documentsui.roots.ProvidersCache;
import com.blabla.documentsui.util.FormatUtils;
import com.blabla.documentsui.OperationDialogFragment;
import com.blabla.documentsui.base.Providers;
...
class CopyJob extends ResolvedResourcesJob {
    ...
    CopyJob(Context service, Listener listener, String id, DocumentStack destination,
        ...
        this(service, listener, id, FileOperationService.OPERATION_COPY, destination, srcs, messenger, features);
    }

    CopyJob(Context service, Listener listener, String id, @FileOperationService.OpType int opType,
    ...
    void finish() {
        try {
            mMessenger.send(Message.obtain(mHandler, FileOperationService.MESSAGE_FINISH, 0, 0));
    ...
    Notification getWarningNotification() {
        ...
        navigateIntent.putExtra(FileOperationService.EXTRA_DIALOG_TYPE, OperationDialogFragment.DIALOG_TYPE_CONVERTED);
        navigateIntent.putExtra(FileOperationService.EXTRA_OPERATION_TYPE, operationType);

        navigateIntent.putParcelableArrayListExtra(FileOperationService.EXTRA_FAILED_DOCS, convertedFiles);

        // TODO: Consider adding a dialog on tapping the notification with a list of
        // converted files.
        final Builder warningBuilder = createNotificationBuilder()
    ...
    private void makeCopyProgress(long bytesCopied) {
        try {
            mMessenger.send(Message.obtain(mHandler, FileOperationService.MESSAGE_PROGRESS,
        ...
        try {
            ...
            while (cursor.moveToNext() && !isCanceled()) {
                if (Document.MIME_TYPE_DIR.equals(
                    ...
                    long size = DocumentInfo.getCursorLong(cursor, Document.COLUMN_SIZE);
    ...
    private static boolean isFileSystemProvider(DocumentInfo info) {
        return Providers.AUTHORITY_STORAGE.equals(info.authority)
            || Providers.AUTHORITY_DOWNLOADS.equals(info.authority);
    }
