...
public class RootInfo implements Durable, Parcelable, Comparable<RootInfo> {
    ...
    public static RootInfo fromRootsCursor(UserId userId, String authority, Cursor cursor) {
        ...
        root.rootId = DocumentInfo.getCursorString(cursor, Root.COLUMN_ROOT_ID);
        root.flags = DocumentInfo.getCursorInt(cursor, Root.COLUMN_FLAGS);
        root.icon = DocumentInfo.getCursorInt(cursor, Root.COLUMN_ICON);
        root.title = DocumentInfo.getCursorString(cursor, Root.COLUMN_TITLE);
        root.summary = DocumentInfo.getCursorString(cursor, Root.COLUMN_SUMMARY);
        root.documentId = DocumentInfo.getCursorString(cursor, Root.COLUMN_DOCUMENT_ID);
        root.availableBytes = DocumentInfo.getCursorLong(cursor, Root.COLUMN_AVAILABLE_BYTES);
        root.mimeTypes = DocumentInfo.getCursorString(cursor, Root.COLUMN_MIME_TYPES);
        root.queryArgs = DocumentInfo.getCursorString(cursor, Root.COLUMN_QUERY_ARGS);
        root.deriveFields();
        return root;
    }
    ...
    private void deriveFields() {
        ...
        if (SharedMinimal.VERBOSE) Log.v(TAG, "Derived fields: " + this);
    }
    ...
    public int compareTo(RootInfo other) {
        ...
        score = Shared.compareToIgnoreCaseNullable(title, other.title);
        if (score != 0) {
            return score;
        }

        return Shared.compareToIgnoreCaseNullable(summary, other.summary);
    }
