...
import com.blabla.documentsui.DragHoverListener;
import com.blabla.documentsui.Injector;
import com.blabla.documentsui.Injector.Injected;
import com.blabla.documentsui.ItemDragListener;
import com.blabla.documentsui.R;
import com.blabla.documentsui.UserPackage;
...
import com.blabla.documentsui.roots.RootsLoader;
import com.blabla.documentsui.util.CrossProfileUtils;
...
public class RootsFragment extends Fragment {
    ...
    List<Item> sortLoadResult(
        ...
        if (SharedMinimal.VERBOSE) Log.v(TAG, "Adding library roots: " + libraries);
        ...
        if (SharedMinimal.VERBOSE) Log.v(TAG, "Adding storage roots: " + storageProviders);
    ...
    private void includeHandlerApps(State state,
            ...
            boolean maybeShowBadge) {
        if (SharedMinimal.VERBOSE) Log.v(TAG, "Adding handler apps for intent: " + handlerAppIntent);
            ...
            for (ResolveInfo info : infos) {
                ...
                if (!myPackageName.equals(packageName)
                    ...
                    if (!CrossProfileUtils.isCrossProfileIntentForwarderActivity(info)) {
                        ...
                        if (SharedMinimal.VERBOSE) Log.v(TAG, "Adding handler app: " + item);
                    }
                }
            }
        }
        ...
        for (RootItem rootItem : otherProviders) {
            ...
            if (UserId.CURRENT_USER.equals(item.userId)) {
                if (SharedMinimal.VERBOSE) Log.v(TAG, "Adding provider : " + item);
                rootList.add(item);
            } else {
                if (SharedMinimal.VERBOSE) Log.v(TAG, "Adding provider to other users : " + item);
                rootListOtherUser.add(item);
            }
        }
...
//                return true;
    ...
    static class ItemComparator implements Comparator<Item> {
        ...
        public int compare(Item lhs, Item rhs) {
            ...
            // Sort by title
            int score = Shared.compareToIgnoreCaseNullable(lhs.title, rhs.title);
            if (score != 0) {
                return score;
            }
            ...
            if (lhs instanceof RootItem) {
                return rhs instanceof RootItem ? Shared.compareToIgnoreCaseNullable(
